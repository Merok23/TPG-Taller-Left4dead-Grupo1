...
TEST_CASE("Infected test, infected follows a soldier through the X border soldier start, infected end.", "[common_infected]") {
...
TEST_CASE("Infected rest, infected follos soldier through the X border, soldier end infected start", "[common_infected]") {
    Game game(CONFIG.scenario_width, CONFIG.scenario_height);
    ...
    game.update();
    std::map<uint32_t, Entity*> entities = game.getEntities();
    REQUIRE(entities[2]->getDirectionOfMovement()->getX() == CONFIG.scenario_width - (1 * CONFIG.common_infected_speed));
}
