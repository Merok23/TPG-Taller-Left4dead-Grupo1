...
Player::Player(std::map<AnimationName, SdlTexture*> &textures): 
    ...
    x(270), y(670), health(100), an_idle(nullptr), an_run(nullptr),
    an_shoot(nullptr), an_die(nullptr)
    {
        auto it = textures.find(AN_IDLE);
        if (it != textures.end())
            an_idle.reset(new Animation(it->second));

        it = textures.find(AN_RUN);
        if (it != textures.end())
            an_run.reset(new Animation(it->second));
        it = textures.find(AN_SHOOT);
        if (it != textures.end())
            an_shoot.reset(new Animation(it->second));

        it = textures.find(AN_DIE);
        if (it != textures.end())
            an_die.reset(new Animation(it->second));
